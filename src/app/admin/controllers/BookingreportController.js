...
/**
 *
 * @annotation adminbookingreport
 * @permission getall,getcounts,remove,
...
module.exports = {
	...
	getAll: async function(ctx) {
		...
		const QUERY = `select b.*,bi.id as bookingtimeid,bi.service_details
					...
					where b.booking_status = 4  and b.payment_status = 1
					order by b.id desc`;
        return db.sequelize.query(QUERY)
        .then( (res) => {
				res[0].map(async(data,index) => {
					let finData = {};
					//	let vprofit = data.actualrate - (data.actualrate * commission/100);
						finData.id = data.id;
						...
						var vendorDetl = JSON.parse(data.vendor_details);
						finData.vendornumber = vendorDetl.length ? vendorDetl[0].vendornumber : "";
						...
						finData.saloonservice_price = saloonPrice;
						var saloonVAT = ((data.vat_percent ? parseInt(data.vat_percent) : 0)/100) * saloonPrice
						finData.saloonservice_vat = saloonVAT; 
						finData.admin_commision = AdminCommision;
						var AdminVAT = Math.floor(((data.vat_percent ? parseInt(data.vat_percent) : 0)/100) * (parseInt(AdminCommision)));
						finData.admin_vat = AdminVAT;
						var serviceTotal = parseInt(saloonPrice) + parseInt(AdminCommision)
						finData.service_total_price = serviceTotal;
						var totalVAT = parseInt(AdminVAT) + parseInt(saloonVAT);
						finData.total_vat = totalVAT;
						finData.totalAmount =  parseInt(serviceTotal) + (parseInt(totalVAT));
						finData.paymentmethod = data.payment_method;
						finalArr.push(finData);
				});
				let getSerData = finalArr.map(async (vendordata,i) => {
					let vendorname = await vendorlang.find(ctx, { query: {vendorid: vendordata.vendor_details[0].id,languageid:1} }).then(async (vendordata) => {
						finalArr[i].vendorname = vendordata.data.length ? vendordata.data[0].vendorname : "";
					});
				})
	...
	getFilterDate: async function(ctx) {
		...
		const QUERY = `select b.*,bi.id as bookingtimeid,bi.service_details
					from tbooking b inner join tbookingtime bi on b.id = bi.bookingid
					where  ( b.service_date  BETWEEN '${ctx.params.fromdate}' and '${ctx.params.todate}')
					and b.booking_status = 4  and b.payment_status = 1`;
        return db.sequelize.query(QUERY)
        .then( (res) => {
				res[0].map((data) => {
					let finData = {};
